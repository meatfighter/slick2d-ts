...
- `Graphics.setColorInverted(...)` / `isColorInverted()` are browser extension APIs for PWA display modes. They expose global WebGL renderer RGB inversion state for subsequent solid, image, font, and flash draws only; alpha and clear color are preserved. Texture batches split on inversion changes, queued batches retain their captured inversion state, and inversion resets at frame and renderer lifecycle boundaries.
