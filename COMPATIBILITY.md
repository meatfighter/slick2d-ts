...
- `Input.pause()` clears browser input state. After `Input.resume()`, the first controller poll establishes the current held-controller baseline without synthesizing one-shot `isControlPressed(...)` state or controller-pressed callbacks. Releases and later fresh presses are reported normally. This prevents controls pressed or held while a host PWA menu owns input from leaking back as fresh gameplay actions.
