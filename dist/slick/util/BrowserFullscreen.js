 ...
 */
export function getBrowserFullscreenCapability(doc = document) {
    const reports = [];
    try {
    ...
    try {
        const webkit = doc.webkitFullscreenEnabled;
...
function hasBrowserFullscreenRequestMethod(doc) {
    let root;
    try {
...
export function getBrowserFullscreenElement(doc = document) {
    try {
...
export function isBrowserFullscreenElement(element, doc = document) {
...
export function exitBrowserFullscreen(doc = document) {
    try {
        if (typeof doc.exitFullscreen === "function") {
            return Promise.resolve(doc.exitFullscreen()).then(() => true);
        }
        const webkitDocument = doc;
...
//# sourceMappingURL=BrowserFullscreen.js.map